...
const AddressModalGuest_vue_vue_type_style_index_0_lang = "";
const _sfc_main = {
  __name: "AddressModalGuest",
  ...
  setup(__props) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      _push(ssrRenderComponent(unref(Link), {
        href: _ctx.route("address-guest.create"),
      ...
      if (!unref(addresses) || ((_a = unref(addresses)) == null ? void 0 : _a.length) < 1) {
        ...
        _push(`<!--[-->`);
        ssrRenderList(unref(addresses), (data, i) => {
          _push(`<div class="col-lg-12 py-2"><hr><div class="d-flex justify-content-between align-items-center mt-2"><span class="small">${ssrInterpolate(JSON.parse(data).receiver)}</span><span>`);
          if (JSON.parse(data).is_primary) {
          ...
          _push(`</span></div><span class="d-block small">${ssrInterpolate(JSON.parse(data).phone)}</span><span class="d-block small">${ssrInterpolate(JSON.parse(data).detail)}</span><span class="d-block small">${ssrInterpolate(`${JSON.parse(data).subdistrict.name}, ${JSON.parse(data).subdistrict.city.name}, ${JSON.parse(data).subdistrict.city.province.name}, ${JSON.parse(data).postal_code}`)}</span></div>`);
        });
        _push(`<!--]-->`);
      }
...
_sfc_main.setup = (props, ctx) => {
  ...
  (ssrContext.modules || (ssrContext.modules = /* @__PURE__ */ new Set())).add("resources/js/Pages/Product/AddressModalGuest.vue");
